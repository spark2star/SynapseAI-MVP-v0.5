#!/usr/bin/env python3
"""
Direct seeding script for mental health symptoms.
This bypasses Alembic and seeds the database directly.
"""
...
def seed_symptoms():
    ...
    try:
        ...
        result = db.execute(text("SELECT COUNT(*) FROM master_symptoms")).scalar()
        if result > 0:
            print(f"⚠️  Database already contains {result} symptoms. Clearing before seeding...")
            db.execute(text("DELETE FROM master_symptoms"))
            db.commit()
        
        # Insert symptoms
        inserted = 0
        for entry in aggregated.values():
            categories_json = json.dumps(sorted(list(entry['categories'])))
            aliases_json = json.dumps([])
            tags_json = json.dumps(sorted(list(entry['tags'])))
            
            db.execute(
                text("""
                    INSERT INTO master_symptoms (id, name, description, categories, aliases, tags, is_active)
                    VALUES (:id, :name, :description, CAST(:categories AS jsonb), CAST(:aliases AS jsonb), CAST(:tags AS jsonb), :is_active)
                """),
                {
                    'id': str(uuid.uuid4()),
                    'name': entry['name'],
        ...
        db.commit()
        print(f"✅ Successfully seeded {inserted} mental health symptoms!")
        
        # Verify
        count = db.execute(text("SELECT COUNT(*) FROM master_symptoms")).scalar()
        print(f"✅ Verification: Database now contains {count} symptoms")
...
if __name__ == "__main__":
    print("🌱 Starting symptom database seeding...")
    seed_symptoms()
    print("\n✨ Seeding complete!")
